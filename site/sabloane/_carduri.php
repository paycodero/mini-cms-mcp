...
<?php foreach ($articole as $a): ?>
  <article class="card<?= !empty($a['eveniment']) ? ' card-eveniment' : '' ?>">
<?php if (($a['imagine'] ?? '') !== ''): $m_card = imagine_masuri((string) $a['imagine']); ?>
    <a class="card-imagine" href="<?= esc(url_element($a)) ?>" tabindex="-1" aria-hidden="true"><img src="<?= esc($a['imagine']) ?>"<?= e_portret((string) $a['imagine']) ? ' class="portret"' : '' ?> alt="" loading="lazy" decoding="async"<?= $m_card ? ' width="' . (int) $m_card['latime'] . '" height="' . (int) $m_card['inaltime'] . '"' : '' ?>></a>
<?php endif; ?>
    <div class="card-text">
